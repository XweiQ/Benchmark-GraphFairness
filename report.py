...
def report1(args, base_path, excel=False):
    """
    select a more adaptive threshold
    """
    if args.model == 'ssf':
        report_path = os.path.join('./report', args.model+'_'+args.encoder, args.dataset, 
                                    'lr'+str(args.lr)+'wd'+str(args.weight_decay),
                                    'drop'+str(args.dropout),
                                    'coeff'+str(args.sim_coeff))
    elif args.model == 'fairgcn':
        report_path = os.path.join('./report', args.model, args.dataset, 
                                    'lr'+str(args.lr)+'wd'+str(args.weight_decay),
                                    'hidden'+str(args.num_hidden)+'drop'+str(args.dropout),
                                    'alpha'+str(args.alpha)+'beta'+str(args.beta))
    elif args.model in ['mlp','gcn']:
        report_path = os.path.join('./report', args.model, 'layers'+str(args.num_layers), args.dataset, 
    ...
    if excel:
        excel_name = 'seed5_adaptivemore_accrocf1.xlsx'
        merge_columns = ['Seed']
        test_metrics = ['ACC', 'ROC', 'F1', 'SP', 'EO']
        max_columns = ['start_epoch', 'stop_epoch', 'max_val_acc', 'max_val_roc', 'max_val_f1']
        df = pd.DataFrame(columns=merge_columns + test_metrics + max_columns)
        data = {'Seed': [args.seed],
        ...
        if os.path.exists(excel_path):
            ...
            existing_df.reset_index(inplace=True)  # 重置索引
            df = existing_df
        df.to_excel(excel_path, index=False)
        #write_dataframe_to_excel(df, excel_path)
        ...
        if args.seed == 5:
            if args.model == 'ssf':
                ...
                earlystop_path = os.path.join('./report', args.model+'_'+args.encoder, args.dataset, excel_name)
            elif args.model == 'fairgcn':
                ...
                earlystop_path = os.path.join('./report', args.model, args.dataset, excel_name)
            elif args.model in ['mlp', 'gcn']:
                numeric_columns = ['layers', 'lr', 'wd', 'drop']
                new_df = pd.DataFrame(columns=numeric_columns)
                row = {'layers': args.num_layers, 'lr': args.lr, 'wd': args.weight_decay, 'drop': args.dropout}
                earlystop_path = os.path.join('./report', args.model, 'layers'+str(args.num_layers), args.dataset, excel_name)    
            ...
            if os.path.exists(earlystop_path):
                ...
                new_df = existing_df
            if args.model == 'ssf':  
                new_df = new_df[['lr', 'wd', 'drop', 'coeff', 'ACC', 'ROC', 'F1', 'SP', 'EO']]
            elif args.model == 'fairgcn':
                new_df = new_df[['lr', 'wd', 'hidden', 'drop', 'alpha', 'beta', 'ACC', 'ROC', 'F1', 'SP', 'EO']]
            elif args.model in ['mlp', 'gcn']:
                new_df = new_df[['layers', 'lr', 'wd', 'drop', 'ACC', 'ROC', 'F1', 'SP', 'EO']]
...
def report(args, base_path, excel=True, draw=False):
    ...
    """
    if args.model == 'ssf':
        report_path = os.path.join('./result', args.model+'_'+args.encoder, args.dataset, 
                                    'lr'+str(args.lr)+'wd'+str(args.weight_decay),
                                    'drop'+str(args.dropout),
                                    'coeff'+str(args.sim_coeff))
    elif args.model == 'fairgcn':
        report_path = os.path.join('./result', args.model, args.dataset, 
                                    'lr'+str(args.lr)+'wd'+str(args.weight_decay),
                                    'hidden'+str(args.num_hidden)+'drop'+str(args.dropout),
                                    'alpha'+str(args.alpha)+'beta'+str(args.beta))
    elif args.model in ['mlp','gcn']:
        # report_path = os.path.join('../'+args.dataset, args.model, 'layers'+str(args.num_layers), 
        #                     'lr'+str(args.lr)+'wd'+str(args.weight_decay), 'drop'+str(args.dropout))
